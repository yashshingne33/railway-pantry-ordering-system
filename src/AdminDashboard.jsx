/**
 * AdminDashboard.jsx
 * Indian Railway Pantry Management System — Commercial Department Dashboard
 * Blue theme · Professional SVG icons · Fully responsive
 * Fixed: QR download, disable, delete vendor/QR, live stats, blue hover rows
 */
...
function DashStatCard({ icon: Icon, label, value, sub, accent = "blue", delay = 0 }) {
  ...
  return (
    <div style={{
      background: "#fff", border: `1px solid #e2e8f0`, borderLeft: `4px solid ${c.border}`,
      borderRadius: 12, padding: "clamp(12px,2.5vw,18px)",
...
/* ── Table with BLUE hover (no colorful rows) ───────────────────────────── */
function DataTable({ headers, children, emptyMsg = "No records" }) {
  return (
...
/* Blue hover row — no color coding */
function TRow({ children, delay = 0 }) {
  const [hovered, setHovered] = useState(false);
  return (
    <tr
      style={{ borderBottom: "1px solid #f1f5f9", background: hovered ? "#eff6ff" : "#fff", transition: "background 0.12s", animation: "fadeUp 0.3s ease both", animationDelay: `${delay}ms` }}
    ...
    <div style={{ display: "flex", gap: 2, padding: 4, background: "#f0f5ff", borderRadius: 12, border: "1px solid #dbeafe", overflowX: "auto", scrollbarWidth: "none", flexWrap: "nowrap" }}>
      {tabs.map(([key, Icon, label, badge]) => (
        <button key={key} onClick={() => onChange(key)} style={{
          padding: "8px clamp(10px,2vw,18px)", borderRadius: 8, border: "none",
          cursor: "pointer", fontWeight: 600, fontSize: "clamp(0.72rem,1.5vw,0.8rem)",
          background: active === key ? "#fff" : "transparent",
          color: active === key ? "#1d4ed8" : "#64748b",
          boxShadow: active === key ? "0 1px 6px rgba(30,78,216,0.12)" : "none",
...
/* ── QR Download helper — renders to off-screen canvas ─────────────────── */
function downloadQRFromRef(svgElement, trainNo) {
  ...
  img.onload = () => {
    ...
    a.download = `QR-Train-${trainNo}.png`;
  ...
  img.onerror = () => {
    ...
    const a = document.createElement("a");
    a.download = `QR-Train-${trainNo}.svg`;
    a.href = url;
    a.click();
...
/* ── QR Card with refs for download ────────────────────────────────────── */
function QRCard({ qr, onZoom, onDelete, onToggle }) {
  ...
  const handleDownload = () => {
    ...
    downloadQRFromRef(svg, qr.trainNo);
  };

  return (
    <div style={{
      background: "#f8faff", borderRadius: 12, border: "1px solid #dbeafe",
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "14px 0 10px", gap: 10 }}>
        <div ref={svgRef} style={{ padding: 10, background: "#fff", borderRadius: 10, border: "1px solid #dbeafe", boxShadow: "0 2px 10px rgba(29,78,216,0.08)" }}>
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", justifyContent: "center" }}>
          ...
          <button onClick={handleDownload} style={qrBtn("#f0fdf4", "#16a34a", "#bbf7d0")}><Icons.Download /> Save</button>
...
/* ── Zoom QR Modal with ref-based download ──────────────────────────────── */
    ...
    <div className="overlay" onClick={onClose}>
      <div className="drawer" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: "clamp(14px,3vw,22px)", textAlign: "center" }}>
          <div ref={svgRef} style={{ padding: 16, background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", display: "inline-block", marginBottom: 14 }}>
          ...
          <button onClick={handleDownload} style={{ width: "100%", padding: 11, background: "linear-gradient(135deg,#0a1628,#1d4ed8)", color: "#fff", border: "none", borderRadius: 10, fontSize: "0.85rem", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
export default function AdminDashboard() {
  ...
  /* ── Live stats — computed from actual orders/vendors ──────────────── */
  ...
  /* ── Per-vendor live sales from orders (match by trainNo) ───────────── */
  const vendorSales  = (trainNo) => orders.filter(o => o.trainNo === (trainNo) && o.status === "Delivered").reduce((s, o) => s + o.total, 0);
  const vendorOrders = (trainNo) => orders.filter(o => o.trainNo === (trainNo)).length;
  ...
  const toggleQR = (qr) => {
    actions.updateQR?.(qr.id, { active: !qr.active });
          ...
          <div style={{ padding: "14px 16px" }}>
            <DataTable headers={["Vendor ID", "Name", "Train", "Live Sales", "Live Orders", "Rating", "Status", "Actions"]}>
              {vendors.map((v, i) => {
                ...
                const liveSales  = vendorSales(tNo);
                const liveOrders = vendorOrders(tNo);
                return (
                  <TRow key={v.id} delay={i * 25}>
                    <TD mono>{v.id}</TD>
                    <TD bold>{v.name}</TD>
                    <TD><CChip>{tNo}</CChip></TD>
                    <TD bold>₹{liveSales.toLocaleString("en-IN")}</TD>
                    <TD>{liveOrders}</TD>
        ...
        <div className="overlay" onClick={() => setShowQR(false)}>
          <div className="drawer" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: "clamp(14px,3vw,20px)", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {newTrain && (
                <div ref={genQRRef} style={{ display: "flex", justifyContent: "center", padding: "1rem", background: "#f0f5ff", borderRadius: 12, border: "1px solid #dbeafe" }}>
                  <div style={{ textAlign: "center" }}>
                    <RealQR url={`${window.location.origin}/app?train=${newTrain}`} size={140} darkColor="#0a1628"/>
                    <p style={{ fontSize: "0.62rem", color: "#64748b", marginTop: 6 }}>/app?train={newTrain}</p>
                  </div>
                </div>
              )}
              <button onClick={genQR} disabled={!newTrain} style={{ width: "100%", padding: "12px", background: newTrain ? "linear-gradient(135deg,#0a1628,#1d4ed8)" : "#e2e8f0", color: newTrain ? "#fff" : "#94a3b8", border: "none", borderRadius: 10, fontSize: "0.88rem", fontWeight: 700, cursor: newTrain ? "pointer" : "not-allowed", boxShadow: newTrain ? "0 4px 16px rgba(29,78,216,0.3)" : "none" }}>
...
const qrBtn = (bg, color, border) => ({
  fontSize: "0.65rem", fontWeight: 700, padding: "4px 10px",
  borderRadius: 6, cursor: "pointer", background: bg, color, border: `1px solid ${border}`,
