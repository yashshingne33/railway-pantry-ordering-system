/**
 * VendorPanel.jsx — FIXED
 *
 * Fix 1: QR download — QRCanvas uses React.forwardRef so qrCanvasRef
 *         points directly to the <canvas> element. downloadCanvasQR
 *         scales it to 400×400 and triggers a real PNG download.
 *
 * Fix 2: Order isolation — myOrders filters by BOTH trainNo AND vendorId.
 *         Orders without a vendorId (passenger app orders) are shown to
 *         the vendor whose train matches.  Vendor-created orders carry
 *         the vendorId and are ONLY shown to that vendor.
 *
 * Fix 3: Done / Packed tabs use the same myOrders slice, so each vendor
 *         only ever sees their own completed orders.
 */
...
/* ── Icons ───────────────────────────────────────────────────────────────── */
const Icons = {
  Train:       () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="3" width="16" height="14" rx="2"/><path d="M4 11h16"/><path d="M12 3v8"/><path d="M8 17l-2 4"/><path d="M18 21l-2-4"/></svg>,
Revenue: () => <span style={{ fontSize: 18, fontWeight: 700, color: "currentColor", lineHeight: 1 }}>₹</span>,  Package:     () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>,
  ...
  Menu:        () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3z"/></svg>,
...
/* ── Status maps ─────────────────────────────────────────────────────────── */
const S_COLOR = { Pending:"#ca8a04", Preparing:"#1d4ed8", Packed:"#7c3aed", Delivered:"#16a34a" };
const S_BG    = { Pending:"#fef3c7", Preparing:"#dbeafe", Packed:"#ede9fe", Delivered:"#dcfce7" };
const S_BD    = { Pending:"#fde68a", Preparing:"#bfdbfe", Packed:"#c4b5fd", Delivered:"#6ee7b7" };
...
let _orderId = 10;
const genId  = () => `ORD-${String(++_orderId).padStart(3,"0")}`;
const nowStr = () => { const d=new Date(); return `${String(d.getHours()).padStart(2,"0")}:${String(d.getMinutes()).padStart(2,"0")} ${String(d.getDate()).padStart(2,"0")}/${String(d.getMonth()+1).padStart(2,"0")}`; };

/* ──────────────────────────────────────────────────────────────────────────
   QRCanvas — forwardRef so parent holds a direct ref to the <canvas> DOM node
   ────────────────────────────────────────────────────────────────────────── */
const QRCanvas = forwardRef(function QRCanvas({ data, size = 190 }, ref) {
  useEffect(() => {
    ...
    const rng = (x, y) => { const v = Math.sin(hash * 9301 + x * 49297 + y * 233) * 49979; return v - Math.floor(v); };
    ...
    for (let r = 0; r < 25; r++) {
      for (let c = 0; c < 25; c++) {
        ...
        if (inFinder) {
          ...
          ctx.fillStyle  = (isBorder || inInner) ? "#0a1628" : "#ffffff";
        } else {
          ctx.fillStyle  = rng(r, c) > 0.45 ? "#0a1628" : "#ffffff";
        }
...
/* ──────────────────────────────────────────────────────────────────────────
   downloadCanvasQR — copies the source canvas → 400×400 output → PNG download
   ────────────────────────────────────────────────────────────────────────── */
function downloadCanvasQR(sourceCanvas, filename) {
  ...
  try {
    ...
    ctx.imageSmoothingEnabled = false;      // keep pixel edges sharp
    ctx.fillStyle = "#ffffff";
...
/* ── Small UI helpers ────────────────────────────────────────────────────── */
const StatusBadge = ({ s }) => (
  <span style={{ fontSize:"0.68rem", fontWeight:700, padding:"3px 9px", borderRadius:20, background:S_BG[s]||"#f1f5f9", color:S_COLOR[s]||"#475569", border:`1px solid ${S_BD[s]||"#e2e8f0"}`, whiteSpace:"nowrap" }}>{s}</span>
);
const CBadge = ({ s }) => (
...
const Pill = ({ children, color="gray" }) => {
  const m = { gray:["#f1f5f9","#374151"], blue:["#dbeafe","#1e40af"], green:["#dcfce7","#15803d"], yellow:["#fef3c7","#92400e"] };
...
const Stars = ({ rating, size=14 }) => (
  <span style={{ color:"#ca8a04", fontSize:size, letterSpacing:1 }}>{"★".repeat(Math.round(rating))}{"☆".repeat(5-Math.round(rating))}</span>
);
...
export default function VendorPanel({ vendor }) {
  ...
  // ── Direct ref to the <canvas> inside the QR modal ──────────────────────
  ...
  useEffect(() => {
    ...
    const unsub = actions.subscribeTrainMenu?.(vendorTrainNo, items => setMenuItems(items || []));
    return () => unsub?.();
  }, [vendorTrainNo]);
  ...
  /* ── Derived collections ─────────────────────────────────────────────── */
  const myTrains     = vendorTrainNo ? [vendorTrainNo] : [];
const myComplaints = complaints.filter(c =>
  myTrains.includes(c.trainNo) &&
  (!c.vendorId || c.vendorId === vendorId)
);
const myFeedback = feedback.filter(f =>
  myTrains.includes(f.trainNo) &&
  ...
  /* ── FIX: vendor-isolated order list ────────────────────────────────────
     Rule:
       • Order has no vendorId   → it came from the passenger app; show it
         to the vendor whose train matches (existing behaviour).
       • Order has a vendorId    → it was created by a specific vendor;
         ONLY show it to that exact vendor.
  ─────────────────────────────────────────────────────────────────────── */
  const myOrders = orders.filter(o => {
    if (!myTrains.includes(o.trainNo)) return false;           // wrong train
    if (o.vendorId && o.vendorId !== vendorId) return false;   // another vendor's order
    return true;
  });
  ...
  const MENU         = menuItems.filter(i => i.available !== false);
  ...
  const remItem  = item => setCart(c => { const n={...c}; n[item.id]>1?n[item.id]--:delete n[item.id]; return n; });

  const cartTotal = editOrder
    ? editItems.reduce((s,it) => s+it.price*(cart[it.id]??it.qty), 0)
    : Object.entries(cart).reduce((s,[id,q]) => s+(MENU.find(m=>m.id===id||m.id===+id)?.price||0)*q, 0);
  const cartCount = editOrder
    ? editItems.reduce((s,it) => s+(cart[it.id]??0), 0)
    : Object.values(cart).reduce((a,b)=>a+b,0);

  const resetForm = () => { setCart({}); setPay("UPI"); };

  /* ── FIX: QR download — reads directly from qrCanvasRef ─────────────── */
  const handleQRDownload = () => {
    const canvas  = qrCanvasRef.current;
    const orderId = (qrOrder || showQR)?.id || "QR";
    const trainNo = (qrOrder || showQR)?.trainNo || vendorTrainNo;
    const ok = downloadCanvasQR(canvas, `QR-${orderId}-Train${trainNo}.png`);
    ok ? showToast("QR downloaded successfully!") : showToast("Download failed — try again", "red");
  };
  ...
  const placeOrder = async () => {
    ...
    const newOrder = {
      id:            genId(),
      trainNo:       vendorTrainNo,
      trainName:     vendorTrainName,
      vendorId,                          // ← stamp vendorId on every vendor-placed order
      seat: "", coach: "",
      items: Object.entries(cart).map(([id,qty]) => {
        const m = MENU.find(m=>m.id===id||m.id===+id);
        return { id, name:m?.name, qty, price:m?.price };
      }),
      total: cartTotal, status:"Pending", time:nowStr(),
      payment: pay, passengerName:"", vendorOrder:true,
    };
  ...
  const saveEdit = async () => {
    ...
    await actions.updateOrder(editOrder.id, { items:updatedItems, total:updatedItems.reduce((s,it)=>s+it.price*it.qty,0) });
  ...
  const remEditItem = it => setCart(c=>{ const n={...c}; n[it.id]=(n[it.id]||0)>1?n[it.id]-1:0; return n; });

  const updateComplaint = async (c, newStatus) => {
    setCLoading(l=>({...l,[c.id]:true}));
    try {
      await actions.updateComplaint(c.id, {
        status: newStatus,
        vendorNote: cNote[c.id]||"",
    ...
    } catch { showToast("Failed to update complaint","red"); }
    setCLoading(l=>({...l,[c.id]:false}));
  };

  /* ── FIX: All tab filters now use myOrders (vendor-isolated) ────────── */
  ...
  const filteredRatings = ratingFilter==="All" ? myFeedback : myFeedback.filter(f=>f.rating===parseInt(ratingFilter));
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#f4f7ff", minHeight:"100vh", padding:"clamp(12px,3vw,22px)" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ margin:0, fontSize:"clamp(1.1rem,3vw,1.45rem)", fontWeight:800, color:"#0a1628" }}>Pantry Panel</h1>
          <p style={{ margin:"3px 0 0", fontSize:"0.72rem", color:"#64748b" }}>{vendor?.name} · {vendorTrainName||`Train ${vendorTrainNo}`} · NGP Division</p>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(clamp(120px,18vw,160px),1fr))", gap:"clamp(8px,2vw,12px)", marginBottom:20 }}>
        {[
          ...
          <div key={s.label} style={{ background:"#fff", borderRadius:12, padding:"clamp(10px,2vw,15px)", border:"1px solid #e2e8f0", borderLeft:`4px solid ${s.border}`, display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, borderRadius:8, background:s.bg, color:s.color, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}><s.icon/></div>
            <div>
      ...
      <div style={{ marginBottom:18, overflowX:"auto", scrollbarWidth:"none" }}>
        <div style={{ display:"flex", gap:2, padding:4, background:"#e8f0ff", borderRadius:12, border:"1px solid #bfdbfe", width:"fit-content", minWidth:"100%" }}>
          {TABS.map(([key,Icon,label,cnt]) => (
            <button key={key} onClick={()=>{ setTab(key); if(key==="ratings") clearNewRatings(); }}
              ...
              {label}
              {cnt>0 && <span style={{ background:tab===key?(key==="complaints"?"#dc2626":key==="ratings"?"#ca8a04":"#1d4ed8"):"#94a3b8", color:"#fff", borderRadius:10, padding:"1px 6px", fontSize:"0.58rem", fontWeight:700 }}>{cnt}</span>}
            </button>
          ))}
        </div>
      </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {myFeedback.length>0 ? (
            ...
            <div style={{ textAlign:"center", padding:"3rem", background:"#fff", borderRadius:12, border:"1px solid #e2e8f0" }}>
              <p style={{ fontWeight:700, color:"#0a1628", margin:"0 0 4px" }}>No ratings yet</p>
              <p style={{ fontSize:"0.78rem", color:"#94a3b8", margin:0 }}>Passenger ratings appear here in real-time</p>
            </div>
          )}
          {myFeedback.length>0&&(
            <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e2e8f0", overflow:"hidden" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 16px", borderBottom:"1px solid #f1f5f9", background:"#f8faff", flexWrap:"wrap", gap:8 }}>
                ...
                <div style={{ display:"flex", gap:4 }}>
                  {["All","5","4","3","2","1"].map(r=>(
                    <button key={r} onClick={()=>setRatingFilter(r)} style={{ padding:"3px 8px", borderRadius:99, border:"1.5px solid", cursor:"pointer", fontSize:"0.62rem", fontWeight:700, background:ratingFilter===r?"#0a1628":"#fff", color:ratingFilter===r?"#fff":"#64748b", borderColor:ratingFilter===r?"#0a1628":"#e2e8f0" }}>{r==="All"?"All":`${r}★`}</button>
                  ))}
                </div>
              </div>
                ...
                {[...filteredRatings].reverse().map((f,i)=>(
                  <div key={f.id} style={{ borderRadius:10, padding:"12px 14px", border:`1px solid ${f.rating>=4?"#bbf7d0":f.rating>=3?"#fde68a":"#fecaca"}`, background:f.rating>=4?"#f0fdf4":f.rating>=3?"#fefce8":"#fef2f2", borderLeft:`4px solid ${f.rating>=4?"#16a34a":f.rating>=3?"#ca8a04":"#dc2626"}`, animation:"fadeUp .25s ease both", animationDelay:`${i*30}ms` }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6, flexWrap:"wrap", gap:6 }}>
                      ...
                      <div style={{ textAlign:"right" }}><Stars rating={f.rating} size={15}/><p style={{ margin:"3px 0 0", fontSize:"0.65rem", fontWeight:700, color:"#64748b" }}>{f.rating}/5</p></div>
                    </div>
                    {f.message&&<div style={{ background:"rgba(255,255,255,.7)", borderRadius:7, padding:"7px 10px", fontSize:"0.78rem", color:"#374151", fontStyle:"italic" }}>"{f.message}"</div>}
                  </div>
                ))}
      ...
      {tab==="complaints"&&(
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom:4 }}>
            {[{label:"Open",count:myComplaints.filter(c=>c.status==="Open").length,color:"#dc2626"},{label:"In Progress",count:myComplaints.filter(c=>c.status==="In Progress").length,color:"#ca8a04"},{label:"Resolved",count:myComplaints.filter(c=>c.status==="Resolved").length,color:"#16a34a"}].map(s=>(
            ...
            <div key={c.id} style={{ background:"#fff", borderRadius:12, padding:16, border:"1px solid #e2e8f0", borderLeft:`4px solid ${C_COLOR[c.status]||"#e2e8f0"}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10, flexWrap:"wrap", gap:8 }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
                    <p style={{ margin:0, fontWeight:700, fontSize:"0.9rem", color:"#0a1628" }}>{c.name}</p><CBadge s={c.status}/>
                  </div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}><Pill color="blue">Train {c.trainNo}</Pill><Pill>Seat {c.seat}</Pill><Pill>{c.time}</Pill></div>
                </div>
              </div>
              ...
              {c.status!=="Resolved"&&(
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  <textarea placeholder="Note for commercial dept. (optional)…" value={cNote[c.id]||""} onChange={e=>setCNote(n=>({...n,[c.id]:e.target.value}))} rows={2} style={{ width:"100%", padding:"8px 10px", borderRadius:8, border:"1px solid #e2e8f0", fontSize:"0.78rem", color:"#0a1628", resize:"vertical", fontFamily:"inherit", boxSizing:"border-box", outline:"none" }}/>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {c.status==="Open"&&<button disabled={cLoading[c.id]} onClick={()=>updateComplaint(c,"In Progress")} style={{ flex:1, minWidth:120, padding:"8px 12px", background:"#fef3c7", color:"#92400e", border:"1px solid #fde68a", borderRadius:8, fontWeight:600, fontSize:"0.78rem", cursor:"pointer", opacity:cLoading[c.id]?0.6:1 }}>{cLoading[c.id]?"Updating…":"Mark In Progress"}</button>}
                    <button disabled={cLoading[c.id]} onClick={()=>updateComplaint(c,"Resolved")} style={{ flex:1, minWidth:120, padding:"8px 12px", background:"#dcfce7", color:"#15803d", border:"1px solid #6ee7b7", borderRadius:8, fontWeight:600, fontSize:"0.78rem", cursor:"pointer", opacity:cLoading[c.id]?0.6:1, display:"flex", alignItems:"center", justifyContent:"center", gap:5 }}><Icons.CheckCircle/>{cLoading[c.id]?"Updating…":"Mark Resolved"}</button>
                  </div>
                </div>
              )}
              {c.status==="Resolved"&&<div style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 12px", background:"#dcfce7", borderRadius:8, border:"1px solid #6ee7b7" }}><Icons.CheckCircle/><p style={{ margin:0, fontSize:"0.75rem", color:"#15803d", fontWeight:600 }}>Resolved{c.resolvedAt?` at ${c.resolvedAt}`:""} — visible to commercial dept.</p></div>}
            </div>
          ))}
        </div>
      )}
      ...
      {!["complaints","ratings","menu"].includes(tab)&&(
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(clamp(260px,35vw,300px),1fr))", gap:"clamp(8px,2vw,14px)" }}>
          {visible.map(o=>(
            <div key={o.id} style={{ background:"#fff", borderRadius:12, padding:"clamp(12px,2vw,16px)", border:"1px solid #e2e8f0", borderTop:`3px solid ${S_COLOR[o.status]||"#1d4ed8"}` }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", borderTop:"1px solid #f1f5f9", paddingTop:8 }}>
                ...
                <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                  {o.status!=="Delivered"&&<button onClick={()=>openEdit(o)} style={iconActBtn("#eff6ff","#1d4ed8","#bfdbfe")}><Icons.Edit/></button>}
                  {o.status==="Pending"&&<button onClick={()=>setConfirmDel(o.id)} style={iconActBtn("#fef2f2","#dc2626","#fecaca")}><Icons.Trash/></button>}
        ...
        <Overlay onClose={()=>{ if(showNew){setShowNew(false);resetForm();}else{setEditOrder(null);setEditItems([]);setCart({});} }}>
          ...
          <div style={{ overflowY:"auto", flex:1, padding:"0 clamp(14px,3vw,20px) 16px" }}>
            <div style={{ marginBottom:12, paddingTop:14 }}>
              ...
              <div style={{ padding:"9px 12px", borderRadius:8, border:"1px solid #dbeafe", background:"#f8faff", fontSize:"0.82rem", color:"#0a1628", fontWeight:600 }}>{vendorTrainNo} — {vendorTrainName}</div>
            </div>
            {!editOrder&&(
              <div style={{ marginBottom:14 }}>
                <label style={labelSt}>Payment</label>
                <div style={{ display:"flex", gap:8 }}>
                  {["UPI","Cash"].map(m=><button key={m} onClick={()=>setPay(m)} style={{ flex:1, padding:"8px", borderRadius:7, border:`1.5px solid ${pay===m?"#1d4ed8":"#e2e8f0"}`, background:pay===m?"#eff6ff":"#fff", color:pay===m?"#1d4ed8":"#64748b", fontWeight:700, fontSize:"0.78rem", cursor:"pointer" }}>{m}</button>)}
                </div>
              </div>
            )}
            {!editOrder&&CATS.length>1&&(
              <div style={{ display:"flex", gap:5, marginBottom:10, flexWrap:"wrap" }}>
                {CATS.map(c=><button key={c} onClick={()=>setMenuCat(c)} style={{ padding:"4px 12px", borderRadius:20, border:"1px solid", cursor:"pointer", fontSize:"0.72rem", fontWeight:600, background:menuCat===c?"#1d4ed8":"#fff", color:menuCat===c?"#fff":"#64748b", borderColor:menuCat===c?"#1d4ed8":"#e2e8f0" }}>{c}</button>)}
              </div>
            )}
            {MENU.length===0&&<div style={{ textAlign:"center", padding:"2rem", background:"#f8faff", borderRadius:10, border:"1px solid #dbeafe", color:"#64748b", fontSize:"0.82rem" }}>No menu items. Add them in the Menu tab.</div>}
                ...
                return (
                  <div key={item.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 10px", borderRadius:8, border:"1px solid #e2e8f0", background:"#fff" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      {item.image?<img src={item.image} alt={item.name} style={{ width:36, height:36, objectFit:"cover", borderRadius:7, border:"1px solid #e2e8f0" }}/>:<span style={{ fontSize:"1.3rem" }}>{item.emoji||"🍽️"}</span>}
                      <div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      {qty>0
                        ?<div style={{ display:"flex", alignItems:"center", gap:6 }}><button onClick={()=>editOrder?remEditItem(item):remItem(item)} style={qtyBtn("#dc2626")}>−</button><span style={{ fontWeight:700, minWidth:18, textAlign:"center", fontSize:"0.85rem" }}>{qty}</span><button onClick={()=>editOrder?addEditItem(item):addItem(item)} style={qtyBtn("#16a34a")}>+</button></div>
          ...
          <div style={{ padding:"12px clamp(14px,3vw,20px)", borderTop:"1px solid #e2e8f0", background:"#fff" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button disabled={!editOrder&&cartCount===0} onClick={editOrder?saveEdit:placeOrder} style={{ background:(!editOrder&&cartCount===0)?"#e2e8f0":"linear-gradient(135deg,#0a1628,#1d4ed8)", color:(!editOrder&&cartCount===0)?"#94a3b8":"#fff", border:"none", borderRadius:8, padding:"10px 20px", fontWeight:700, fontSize:"0.85rem", cursor:"pointer" }}>
      ...
      {/* ── QR Modal — QRCanvas ref wired directly to qrCanvasRef ── */}
      {activeQR&&(
        <Overlay onClose={()=>{setShowQR(null);setQrOrder(null);}} centered>
          ...
          <div style={{ padding:"clamp(16px,3vw,20px)", textAlign:"center" }}>
            <div style={{ background:"#fff", borderRadius:12, padding:16, display:"inline-block", border:"2px solid #dbeafe", marginBottom:14, boxShadow:"0 4px 20px rgba(29,78,216,0.1)" }}>
              {/* forwardRef canvas — qrCanvasRef IS the canvas element */}
              <QRCanvas ref={qrCanvasRef} data={activeQR.id} size={190}/>
            </div>
            <div style={{ background:"#f8faff", borderRadius:10, padding:"10px 16px", border:"1px solid #dbeafe", marginBottom:12, display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
              ...
              <button onClick={()=>navigator.clipboard?.writeText(activeQR.id)} style={{ background:"#eff6ff", color:"#1d4ed8", border:"1px solid #bfdbfe", borderRadius:6, padding:"3px 9px", fontSize:"0.68rem", cursor:"pointer", fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><Icons.Copy/>Copy</button>
            </div>
            ...
            <button onClick={handleQRDownload} style={{ width:"100%", padding:"10px", background:"#f0fdf4", color:"#16a34a", border:"1px solid #bbf7d0", borderRadius:9, fontWeight:700, fontSize:"0.82rem", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
              <Icons.Download/>Download QR PNG
            </button>
          </div>
        </Overlay>
      )}
...
function DrawerHeader({ title, onClose }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"16px clamp(14px,3vw,20px)", borderBottom:"1px solid #e2e8f0", background:"linear-gradient(135deg,#f8faff,#f0f5ff)", borderRadius:"14px 14px 0 0", flexShrink:0 }}>
      ...
      <button onClick={onClose} style={{ background:"#f1f5f9", border:"1px solid #e2e8f0", borderRadius:7, width:30, height:30, cursor:"pointer", color:"#475569", display:"flex", alignItems:"center", justifyContent:"center" }}>
